...
QuickSysExPartBlock::QuickSysExPartBlock(SynthParts part) :
...
QuickSysExRhythmBlock::QuickSysExRhythmBlock(RhythmGroup rhyGrp) :
...
QuickSysExSequencerBlock::QuickSysExSequencerBlock() :
...
Parameter* QuickSysExSequencerBlock::getParameter(RhythmGroup rhyGrp)
{
	switch (rhyGrp)
	{
...
SyxMsg* QuickSysExSequencerBlock::createMuteQuickSysExSendMsg(Parameter* param)
{
...
QuickSysExBlock::QuickSysExBlock() :
	GrooveboxMemoryBlock(0x00000000, "Quick SysEx Block", "3")
{
	...
	addSubBlock(new QuickSysExRhythmBlock(BD));
	addSubBlock(new QuickSysExRhythmBlock(SD));
	addSubBlock(new QuickSysExRhythmBlock(HH));
	addSubBlock(new QuickSysExRhythmBlock(CLP));
	addSubBlock(new QuickSysExRhythmBlock(CYM));
	addSubBlock(new QuickSysExRhythmBlock(TOM_PERC));
...
{
	unsigned short addr = msg->getAddressOffset();
	uint32 size;
	uint8* data;
	msg->getSysExDataArrayPtr(&data, size);
	uint8 aaAddressStatusChannel = addr >> 8;
	uint8 bbAddressDataH = addr & 0xFF;
	uint8 ccDataL = data[0];
	uint8 ddDataE = data[1];

	if (aaAddressStatusChannel >= 0x30 && aaAddressStatusChannel <= 0x36)
	{
		SynthParts part = (SynthParts)(aaAddressStatusChannel & 0x0F);
		getQuickSysExPartBlock(part)->getParameter(bbAddressDataH)->setValue(ccDataL, Parameter::Init);
	}
	else if (aaAddressStatusChannel == 0x39)
	{
		RhythmGroup rhyGrp = (RhythmGroup)ddDataE;
		if (rhyGrp == ALL)
			for (int i = 0; i <= 7; i++)
			{
				rhyGrp = (RhythmGroup)i;
				getQuickSysExRhythmBlock(rhyGrp)->getParameter(bbAddressDataH)->setValue(ccDataL, Parameter::Init);
			}
		else
		{
			getQuickSysExRhythmBlock(rhyGrp)->getParameter(bbAddressDataH)->setValue(ccDataL, Parameter::Init);
	...
	else if (aaAddressStatusChannel == 0x70)
	{
		if (bbAddressDataH == 1)
		{
...
QuickSysExRhythmBlock* QuickSysExBlock::getQuickSysExRhythmBlock(RhythmGroup rhyGroup)
{
	switch (rhyGroup)
	{
	case BD: return dynamic_cast<QuickSysExRhythmBlock*>(m_subBlocks[7]);
	case SD: return dynamic_cast<QuickSysExRhythmBlock*>(m_subBlocks[8]);
	case HH: return dynamic_cast<QuickSysExRhythmBlock*>(m_subBlocks[9]);
	case CLP: return dynamic_cast<QuickSysExRhythmBlock*>(m_subBlocks[10]);
	case CYM: return dynamic_cast<QuickSysExRhythmBlock*>(m_subBlocks[11]);
	case TOM_PERC: return dynamic_cast<QuickSysExRhythmBlock*>(m_subBlocks[12]);
	case HIT: return dynamic_cast<QuickSysExRhythmBlock*>(m_subBlocks[13]);
	case OTHERS: return dynamic_cast<QuickSysExRhythmBlock*>(m_subBlocks[14]);
	case ALL:  default: return nullptr;
	}
}

QuickSysExSequencerBlock* QuickSysExBlock::getQuickSysExSequencerBlock()
{
